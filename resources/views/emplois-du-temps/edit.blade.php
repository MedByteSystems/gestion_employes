<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier une activité de l\'emploi du temps') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('equipes.emplois-du-temps.update', [$equipe, $emploiDuTemps]) }}">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-4">
                            <label for="titre" class="block text-sm font-medium text-gray-700">Titre de l'activité</label>
                            <input type="text" name="titre" id="titre" value="{{ old('titre', $emploiDuTemps->titre) }}" required
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $emploiDuTemps->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div>
                                ...
                                <select name="jour_semaine" id="jour_semaine" required
                                    ...
                                    @foreach ($joursDelaSemaine as $jour)
                                        <option value="{{ $jour }}" {{ old('jour_semaine', $emploiDuTemps->jour_semaine) == $jour ? 'selected' : '' }}>
                                            {{ $jour }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="type_activite" id="type_activite"
                                    ...
                                    @foreach ($typesActivite as $type)
                                        <option value="{{ $type }}" {{ old('type_activite', $emploiDuTemps->type_activite) == $type ? 'selected' : '' }}>
                                            {{ $type }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="time" name="heure_debut" id="heure_debut" 
                                    value="{{ old('heure_debut', \Carbon\Carbon::parse($emploiDuTemps->heure_debut)->format('H:i')) }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                @error('heure_debut')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="time" name="heure_fin" id="heure_fin" 
                                    value="{{ old('heure_fin', \Carbon\Carbon::parse($emploiDuTemps->heure_fin)->format('H:i')) }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                @error('heure_fin')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="lieu" id="lieu" value="{{ old('lieu', $emploiDuTemps->lieu) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('lieu')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="recurrent" id="recurrent" value="1" 
                                    {{ old('recurrent', $emploiDuTemps->recurrent) ? 'checked' : '' }}
                        ...
                        <div id="date_specifique_container" class="mb-4" style="display: none;">
                            ...
                            <input type="date" name="date_specifique" id="date_specifique" 
                                value="{{ old('date_specifique', $emploiDuTemps->date_specifique ? $emploiDuTemps->date_specifique->format('Y-m-d') : '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
